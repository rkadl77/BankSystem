...
final class APIClient: NSObject, URLSessionDelegate {
    ...
    func patch<B: Encodable, R: Decodable>(_ url: String, body: B) async throws -> R {
        var r = make(url, "PATCH"); r.httpBody = try encoder.encode(body)
        return try await perform(r)
    }
    func patchVoid<B: Encodable>(_ url: String, body: B) async throws {
        var r = make(url, "PATCH"); r.httpBody = try encoder.encode(body)
        try await performVoid(r)
    }
    ...
    private func validate(_ res: URLResponse, _ data: Data) throws {
        ...
        guard (200...299).contains(h.statusCode) else {
            if h.statusCode == 401 { throw NetworkError.unauthorized }
