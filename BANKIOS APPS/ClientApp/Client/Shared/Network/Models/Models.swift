...
struct TransactionDTO: Codable, Identifiable {
    ...
    var conversionInfo: String? {
        guard let rate = conversionRate, let from = originalCurrency, let to = targetCurrency,
              from != to else { return nil }
        return "Конвертация \(from)→\(to), курс \(String(format: "%.4f", rate))"
    }
...
struct CreditDTO: Codable, Identifiable {
    ...
    var isOverdue: Bool { status.lowercased() == "overdue" }
...
struct CreditTariffDTO: Codable, Identifiable, Hashable {
    let id: UUID
    let name: String
    let interestRate: Double
    let description: String
