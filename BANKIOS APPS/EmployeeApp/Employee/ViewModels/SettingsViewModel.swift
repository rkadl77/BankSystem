...
final class SettingsViewModel: ObservableObject {
    ...
    func setTheme(_ dark: Bool) {
        isDarkTheme = dark
        Task {
            do { _ = try await SettingsService.shared.updateTheme(userId: userId,
    ...
    func toggleAccountVisibility(_ accountId: UUID) {
        if hiddenAccountIds.contains(accountId) { hiddenAccountIds.remove(accountId) }
        else { hiddenAccountIds.insert(accountId) }
        let ids = Array(hiddenAccountIds)
        Task {
            do { _ = try await SettingsService.shared.updateHiddenAccounts(userId: userId, accountIds: ids) }
            catch { errorMessage = error.localizedDescription }
        }
    }

    func isHidden(_ accountId: UUID) -> Bool { hiddenAccountIds.contains(accountId) }
}
