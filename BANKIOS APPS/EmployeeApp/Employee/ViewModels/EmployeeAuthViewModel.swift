...
final class EmployeeAuthViewModel: ObservableObject {
    ...
    private let callbackScheme = "bankapp-employee"
    private let contextProvider = EmployeeWebAuthContextProvider()
    ...
    func startWebRegister() {
        openWebPage(path: "/register.html")
    }

    func startWebRegisterEmployee() {
        openWebPage(path: "/register-employee.html")
    }

    func startWebLogin() {
        errorMessage = nil
        let redirect = "\(callbackScheme)://auth/callback"
        guard var components = URLComponents(string: "\(authWebBase)/login.html") else { return }
    ...
    private func openWebPage(path: String) {
        let redirect = "\(callbackScheme)://auth/callback"
        guard var components = URLComponents(string: "\(authWebBase)\(path)") else { return }
        components.queryItems = [URLQueryItem(name: "redirect_uri", value: redirect)]
        guard let url = components.url else { return }
    ...
    private func handleCallback(url: URL?, error: Error?) async {
        ...
        do {
            ...
            guard user.role == "employee" || user.role == "admin" else {
                errorMessage = "Нет прав сотрудника"
                TokenStore.shared.token = nil; return
            }
    ...
    private func restoreSession(userId: UUID) async {
        do {
            ...
            guard user.isActive, (user.role == "employee" || user.role == "admin") else {
                TokenStore.shared.token = nil; return
            }
            let employees = try await UserService.shared.getEmployees()
            currentEmployee = employees.first { $0.userId == user.id }
...
final class EmployeeWebAuthContextProvider: NSObject, ASWebAuthenticationPresentationContextProviding {
